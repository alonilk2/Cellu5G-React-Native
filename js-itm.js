...
JSITM.LatLng.prototype.toString = function(){
    ...
    return round(JSITM.lat, JSITM.precision) + " " + round(JSITM.lng, JSITM.precision);
}

/**
 * Creates a new LatLng by converting coordinates from a source ellipsoid to a target one. <p>
 * JSITM process involves:<p>
 *   1. converting the angular latlng to cartesian coordinates using latLngToPoint()<p>
 *   2. translating the XYZ coordinates using a translation, with special values supplied to JSITM translation.<p>
...
JSITM.LatLng.prototype.convertGrid = function(from, to, translation){
    ...
    var point = from.latLngToPoint(JSITM, 0);
...
JSITM.Point.prototype.toString = function(){
    ...
    return Math.round(JSITM.x) + " " + Math.round(JSITM.y);
}
...
JSITM.Translation.prototype.translate = function(point){
    return new JSITM.Point(point.x + JSITM.dx, point.y + JSITM.dy, point.z + JSITM.dz);
}
...
JSITM.Translation.prototype.inverse = function(){
    return new JSITM.Translation(-JSITM.dx, -JSITM.dy, -JSITM.dz);
}
...
JSITM.Ellipsoid = function(a, b){
    ...
    JSITM.e2 = (Math.pow(a, 2) - Math.pow(b, 2)) / Math.pow(a, 2);
...
JSITM.Ellipsoid.prototype.pointToLatLng = function(point){
    ...
    var radlat1 = Math.atan2(point.z, (RootXYSqr * (1 - JSITM.e2)));
    
    do {
        var V = JSITM.a / (Math.sqrt(1 - (JSITM.e2 * Math.pow(Math.sin(radlat1), 2))));
        var radlat2 = Math.atan2((point.z + (JSITM.e2 * V * (Math.sin(radlat1)))), RootXYSqr);
...
JSITM.Ellipsoid.prototype.latLngToPoint = function(latlng){
    ...
    // Compute nu
    var V = JSITM.a / (Math.sqrt(1 - (JSITM.e2 * (Math.pow(Math.sin(radlat), 2)))));
    
    // Compute XYZ
    var x = (V + latlng.alt) * (Math.cos(radlat)) * (Math.cos(radlng));
    var y = (V + latlng.alt) * (Math.cos(radlat)) * (Math.sin(radlng));
    var z = ((V * (1 - JSITM.e2)) + latlng.alt) * (Math.sin(radlat));
...
JSITM.TM = function(ellipsoid, e0, n0, f0, lat0, lng0){
    ...
    JSITM.radlat0 = lat0 * (Math.PI / 180);
    JSITM.radlng0 = lng0 * (Math.PI / 180);
    
    JSITM.af0 = ellipsoid.a * f0;
    JSITM.bf0 = ellipsoid.b * f0;
    JSITM.e2 = (Math.pow(JSITM.af0, 2) - Math.pow(JSITM.bf0, 2)) / Math.pow(JSITM.af0, 2);
    JSITM.n = (JSITM.af0 - JSITM.bf0) / (JSITM.af0 + JSITM.bf0);
    JSITM.n2 = JSITM.n * JSITM.n; //for optimizing and clarity of Marc()
    JSITM.n3 = JSITM.n2 * JSITM.n; //for optimizing and clarity of Marc()
 ...
 * @return {Number}
 */
JSITM.Marc = function(radlat){
    
    return (
		JSITM.bf0 * (
			((1 + JSITM.n + ((5 / 4) * JSITM.n2) + ((5 / 4) * JSITM.n3)) * (radlat - JSITM.radlat0)) - 
			(((3 * JSITM.n) + (3 * JSITM.n2) + ((21 / 8) * JSITM.n3)) * (Math.sin(radlat - JSITM.radlat0)) * (Math.cos(radlat + JSITM.radlat0))) + 
			((((15 / 8) * JSITM.n2) + ((15 / 8) * JSITM.n3)) * (Math.sin(2 * (radlat - JSITM.radlat0))) * (Math.cos(2 * (radlat + JSITM.radlat0)))) - 
			(((35 / 24) * JSITM.n3) * (Math.sin(3 * (radlat - JSITM.radlat0))) * (Math.cos(3 * (radlat + JSITM.radlat0))))
		)
	);
...
JSITM.TM.prototype.InitialLat = function(y){
    ...
    var radlat1 = ((y - JSITM.n0) / JSITM.af0) + JSITM.radlat0;

    var M = JSITM.Marc(radlat1);
    
    var radlat2 = ((y - JSITM.n0 - M) / JSITM.af0) + radlat1;
    
    while (Math.abs(y - JSITM.n0 - M) > 0.00001) {
        radlat2 = ((y - JSITM.n0 - M) / JSITM.af0) + radlat1;
        M = JSITM.Marc(radlat2);
        radlat1 = radlat2;
    }
    return radlat2;
}
...
JSITM.TM.prototype.unproject = function(point){
    ...
    //Compute Et
    var Et = point.x - JSITM.e0;
    
    //Compute initial value for latitude (PHI) in radians
    var PHId = JSITM.InitialLat(point.y);
    
    //Compute nu, rho and eta2 using value for PHId
    var nu = JSITM.af0 / (Math.sqrt(1 - (JSITM.e2 * (Math.pow(Math.sin(PHId), 2)))));
    var rho = (nu * (1 - JSITM.e2)) / (1 - (JSITM.e2 * Math.pow(Math.sin(PHId), 2)));
    var eta2 = (nu / rho) - 1;
    ...
    var lng = (180 / Math.PI) * (JSITM.radlng0 + (Et * X) - (Math.pow(Et, 3) * XI) + (Math.pow(Et, 5) * XII) - (Math.pow(Et, 7) * XIIA));
...
JSITM.TM.prototype.project = function(latlng){
    ...
    var nu = JSITM.af0 / (Math.sqrt(1 - (JSITM.e2 * Math.pow(Math.sin(RadPHI), 2))));
    var rho = (nu * (1 - JSITM.e2)) / (1 - (JSITM.e2 * Math.pow(Math.sin(RadPHI), 2)));
    var eta2 = (nu / rho) - 1;
    var p = RadLAM - JSITM.radlng0;
    var M = JSITM.Marc(RadPHI);
    ...
    var x = JSITM.e0 + (p * IV) + (Math.pow(p, 3) * V) + (Math.pow(p, 5) * VI);
...
JSITM.itm2gps = function(point){
    var latlng = JSITM.ITM.unproject(point); //however, latlng is still on GRS80!
    return latlng.convertGrid(JSITM.GRS80, JSITM.WGS84, JSITM.GRS80toWGS84);
}
...
JSITM.itmRef2gps = function(s){
	var point = JSITM.itmRef2Point(s);
	return JSITM.itm2gps(point);
}
...
JSITM.itmRef2gpsRef = function(s){
	return JSITM.itmRef2gps(s).toString();
}
...
export default JSITM.gpsRef2itm = function(s){
	var latlng = JSITM.latlngFromString(s);
    var wgs84 = latlng.convertGrid(JSITM.WGS84, JSITM.GRS80, JSITM.WGS84toGRS80); //first convert to GRS80
    console.log("wgs:"+wgs84);
    return JSITM.ITM.project(wgs84);
}
...
JSITM.gpsRef2itmRef = function(s, precision){
	return JSITM.point2ItmRef(JSITM.gpsRef2itm(s), precision || 6);
}
